...
__all__ = ['MakeBorderMap', 'MakeBorderMapMulti']
...
class MakeBorderMap(object):
    ...
    def draw_border_map(self, polygon, canvas, mask):
        ...
        try:
            ...
            # print(polygon_shape.area, polygon.shape, polygon, distance)
...
class MakeBorderMapMulti(object):
    def __init__(self,
                 shrink_ratio=0.4,
                 thresh_min=0.3,
                 thresh_max=0.7,
        ...
        self.all_class_idxs = [i for k in class_idxs for i in k]
    ...
    def __call__(self, data):
        ...
        if self.class_idxs is None:
            ...
            canvas_list, mask_list = [], []
            for class_idx in self.class_idxs:  
                canvas = np.zeros(img.shape[:2], dtype=np.float32)
                mask = np.zeros(img.shape[:2], dtype=np.float32)
                for text_poly, text, ignore_tag in zip(text_polys, texts, ignore_tags):
                    if text not in self.all_class_idxs:
                        print(text)
                        assert text in self.all_class_idxs
                    if ignore_tag is True or text not in class_idx:
                        continue
                    self.draw_border_map(text_poly, canvas, mask=mask)
                canvas = canvas * (self.thresh_max - self.thresh_min) + self.thresh_min
                canvas_list.append(canvas)
                mask_list.append(mask)
            data['threshold_map'] = np.asarray(canvas_list)
            data['threshold_mask'] = np.asarray(mask_list)
            return data    
