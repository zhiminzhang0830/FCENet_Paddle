...
import re
def load_dygraph_params_new(config, model, logger, optimizer):
    ckp = config['Global']['checkpoints']
    if ckp and os.path.exists(ckp + ".pdparams"):
        pre_best_model_dict = init_model(config, model, optimizer)
        return pre_best_model_dict
        ...
        for k1 in list(state_dict.keys()):
            new_k1 = re.sub('heads.\d+.', '', k1)
            if k1 in list(params.keys()) and list(state_dict[k1].shape) == list(params[k1].shape):
                new_state_dict[k1] = params[k1]
            elif re.sub('heads.\d+.', '', k1) in list(params.keys()):
                new_k1 = re.sub('heads.\d+.', '', k1)
                if list(state_dict[k1].shape) == list(params[new_k1].shape):
                    new_state_dict[k1] = params[new_k1]
                    logger.info(
                        f"The params {k1} {state_dict[k1].shape} matched with loaded params {new_k1} {params[new_k1].shape} !"
                )
                else:
                    logger.info(
                        f"The shape of model params {k1} {state_dict[k1].shape} not matched with loaded params {new_k1} {params[new_k1].shape} !"
                )
            else:
                logger.info(
