...
class FCELoss(nn.Layer):
    ...
    def forward_single(self, pred, gt):
        ...
        if tr_train_mask.sum().item() > 0:
            ...
            loss_tcl_neg = F.cross_entropy(tcl_pred.masked_select(tr_neg_mask2).reshape([-1, 2]),
        ...
        loss_reg_y = paddle.to_tensor(0.).astype('float32')
        if tr_train_mask.sum().item() > 0:
            weight = (tr_mask.masked_select(tr_train_mask.astype('bool')).astype('float32') +
                      tcl_mask.masked_select(tr_train_mask.astype('bool')).astype('float32')) / 2
    ...
    def ohem(self, predict, target, train_mask):
        ...
        if n_pos.item() > 0:
            ...
            loss_neg = F.cross_entropy(
                predict.masked_select(neg2).reshape([-1, 2]), target.masked_select(neg).astype('int64'), reduction='none')
            n_neg = min(
    ...
    def fourier2poly(self, real_maps, imag_maps):
        ...
        transform_matrix = 2 * np.pi / self.num_sample * paddle.matmul(
            k_vect, i_vect)
